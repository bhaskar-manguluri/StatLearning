...
tree.carseats
```
Ecaluating performance using validation set.
```{r}
set.seed(2)
...
```{r}
par(mfrow=c(1,2))
plot(cv.carseats$size,cv.carseats$dev,type="b",main="Error ~ size")
...
Checking the performance of best model from crossvalidation
```{r}
